...
private:
	...
	int32_t add(dataNode &x, const Key &k, const V &val) {
		...
		if (x.size && i > 0 && !cmp(k, x.key[i - 1]) && !cmp(x.key[i - 1], k)) throw repetition();
...
public:
	...
	public:
		...
		bool operator==(const iterator &rhs) const {
			...
			if (now.pos == rhs.now.pos && now.pos == 0) return 1;
			if (now.pos == 0 || rhs.now.pos == 0) return 0;
			if (!(*mine).cmp(now.key[cur], rhs.now.key[cur]) && !(*mine).cmp(rhs.now.key[cur], now.key[cur])) return 1;		
			// cout<<"!!"<<endl;
			return 0;
		}
